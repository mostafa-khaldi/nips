...
- For each gateway, an identifier that represents the gateway Nostr pubkey
...
- Represents the Nostr pubkey of the payment gateway. Clients SHOULD use this pubkey to resolve the gateway’s profile metadata via standard Nostr events (e.g., kind 0). When included in payment-related events (such as Lightning invoice definitions), this tag allows the referenced gateway to subscribe to relevant events, detect incoming payments, and perform subscriber validation.
